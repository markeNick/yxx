...
Page({
    ...
    getList: function(isConcat, page) {
        ...
        wx.request({
            url: `https://www.yxxcloud.cn/api/selectAllMySaleGoods?currentPage=${newPage}`,
            data: {
            ...
            success: (result) => {
                const mysalelist = result.data.mysalelist;
                if (!mysalelist) {
                ...
                let newlist = '';
                if (Concat) {
                    newlist = _this.data.goodsList.concat(mysalelist)
                } else {
                    newlist = mysalelist
                }
                //停止加载动画
